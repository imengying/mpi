...
`↑`/`↓` 在**菜单开着时选菜单**（`Tab`/`Shift+Tab` 同样可以），菜单关着时翻输入历史。
...
按 `Ctrl+O` 展开输出、输入 `/` 看菜单。**光标在整个回合里停在原地不动**：
上面的预览区是固定高度的，转圈那一拍只重画它自己和它上面的行，token 按批到达、
一批只重画一次——不然输入行和光标会被反复擦掉重画，看起来就是在抖。
活区里每一行也都按终端宽度折好：交给终端折的话，它会折成两行而程序只算一行，
于是每重绘一次整块就往下爬一行，屏幕上的表现是**同一行文字连续重复十几次**，
很像模型在重复输出。此时按 `Enter` 提交的行会排在当前这一轮后面，
...
任务列表）、表格（含对齐）、引用、分隔线、链接。代码块按语言做轻量高亮（关键字、字符串、
注释、数字），**手写**，不引入语法高亮库；认不出的语言保持原色，不做自动探测。
...
**摘要请求也受窗口限制**：它只发「用户消息 + 助手正文 + 工具调用与结果」，
**不发思考过程**（推理轨迹是模型的草稿，且体积极大——实测能占整个请求的 78%），
单条助手正文与工具结果也各有上限；整段还要再按「模型窗口的三分之一」截断，
从最旧的对话开始丢。所以 `/compact` 不会因为在长会话上「请求本身就超窗口」而失败。
检查点保留全部用户消息，所以用户说过的意图不会丢。
